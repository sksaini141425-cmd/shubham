...
loss_streak = {"count": 0, "cooldown_until": None} # count of consecutive losses
...
def save_bot_state():
    try:
        with open(STATE_FILE, "w") as f:
            data = {
                "allow_new_trades": allow_new_trades["value"],
                "loss_streak_count": loss_streak["count"],
                "cooldown_until": loss_streak["cooldown_until"].isoformat() if loss_streak["cooldown_until"] else None
            }
            json.dump(data, f)
    ...
    if os.path.exists(STATE_FILE):
        try:
            with open(STATE_FILE, "r") as f:
                ...
                loss_streak["count"] = data.get("loss_streak_count", 0)
                cooldown_str = data.get("cooldown_until")
                if cooldown_str:
                    loss_streak["cooldown_until"] = datetime.fromisoformat(cooldown_str)
                logger.info(f"Loaded bot state: allow_new_trades={allow_new_trades['value']}, loss_streak={loss_streak['count']}")
        except Exception as e:
        ...
        try:
            ...
            with symbol_states_lock:
                symbol_states[symbol] = {
                    ...
                    "entry": round(exchange.entry_price, 8) if exchange.is_in_position else 0,
                    "size": round(exchange.position_size, 8) if exchange.is_in_position else 0,
                    "upnl": round(upnl_net, 6),
                    "margin": round(getattr(exchange, 'entry_margin', 0.0), 4) if exchange.is_in_position else 0,
            ...
            if exchange.is_in_position:
                ...
                if close_position:
                    ...
                    # --- LOSS STREAK / CIRCUIT BREAKER LOGIC ---
                    global loss_streak
                    if net_pnl <= 0:
                        loss_streak["count"] += 1
                        if loss_streak["count"] >= 3:
                            # 1 hour cooldown
                            loss_streak["cooldown_until"] = datetime.utcnow() + timedelta(hours=1)
                            logger.warning(f"🛑 CIRCUIT BREAKER TRIGGERED: {loss_streak['count']} consecutive losses. Cooling down until {loss_streak['cooldown_until']}")
                            safe_send_message(notifier, f"🛑 *Circuit Breaker Triggered*\n{loss_streak['count']} consecutive losses detected. Trading paused for 1 hour until {loss_streak['cooldown_until'].strftime('%H:%M:%S')} UTC.")
                    else:
                        # Reset on win
                        if loss_streak["count"] > 0:
                            logger.info(f"✅ Loss streak reset after win on {symbol}.")
                        loss_streak["count"] = 0
                        loss_streak["cooldown_until"] = None
                    
                    save_bot_state() # Persist the new streak count
                    # ------------------------------------------
...
def run_paper_trading():
    ...
    # --- PROFIT COMPOUNDING LOGIC ---
    # If we are in paper mode, resume from the last known balance in the log
    if not USE_REAL_EXCHANGE and global_account.get_cash() > INITIAL_CAPITAL:
        logger.info(f"📈 Resuming with COMPOUNDED capital: ${global_account.get_cash():.2f} (Profit from previous sessions included)")
        INITIAL_CAPITAL = global_account.get_cash()
    # --------------------------------
    ...
    try:
        while True:
            with symbol_states_lock:
                ...
                # Compute total unrealized PnL and locked margin across ALL open positions
                total_upnl = 0.0
                total_margin = 0.0
                for s in symbol_states.values():
                    if s.get('direction', 'FLAT') != 'FLAT':
                        total_upnl += s.get('upnl', 0)
                        total_margin += s.get('margin', 0)
                
                dashboard_state["trades"] = list(global_account.trade_history)
            with active_trades_lock:
                open_count = len(active_trades)
            
            # live_balance = available_cash + unrealized_pnl + locked_margin
            realized_cash = global_account.get_cash()
            live_balance = realized_cash + total_upnl + total_margin
            
            dashboard_state["total_upnl"] = round(total_upnl, 6)
            dashboard_state["total_margin"] = round(total_margin, 6)
            dashboard_state["realized_cash"] = round(realized_cash, 6)
            dashboard_state["live_balance"] = round(live_balance, 6)
            dashboard_state["initial_capital"] = INITIAL_CAPITAL
