...
class EliteScalperStrategy(BaseStrategy):
    ...
    def generate_signals(self, data_list):
        ...
        for i in range(1, len(data_list)):
            ...
            ema50, vol_sma = d.get('EMA_50'), d.get('VOL_SMA')
            d['signal'] = 'NONE'
            if None in [rsi, macd_hist, prev_macd_hist, ema50, vol_sma]: continue
            
            # Professional Scalping: Trend + RSI Pullback + Momentum Shift
            # LONG: Price above EMA 50 (Uptrend) + RSI < 40 (Pullback) + MACD turning up
            if price > ema50 and rsi < 40 and macd_hist > prev_macd_hist:
                d['signal'] = 'LONG'
            # SHORT: Price below EMA 50 (Downtrend) + RSI > 60 (Pullback) + MACD turning down
            elif price < ema50 and rsi > 60 and macd_hist < prev_macd_hist:
                d['signal'] = 'SHORT'
        return data_list
...
class Scalper70Strategy(BaseStrategy):
    ...
    def generate_signals(self, data_list):
        ...
        for i in range(1, len(data_list)):
            ...
            if price > ema200 and (rsi < 35 or price < ema50) and macd_hist > prev_macd_hist and vol > (vol_sma * 1.2):
                d['signal'] = 'LONG'
            ...
            elif price < ema200 and (rsi > 65 or price > ema50) and macd_hist < prev_macd_hist and vol > (vol_sma * 1.2):
                d['signal'] = 'SHORT'
